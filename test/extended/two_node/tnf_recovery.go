...
const (
	...
	memberIsDownTimeout          = 30 * time.Second
...
var _ = g.Describe("[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Disruptive] Two Node with Fencing etcd recovery", func() {
	...
	g.It("Should recover from ungraceful node shutdown with etcd member re-addition", func() {
		g.By(fmt.Sprintf("Shutting down %s ungracefully", targetNode.Name))
		...
		// Ensure the etcd server is down consistently
		o.Consistently(func() error {
			err := helpers.EnsureHealthyMember(g.GinkgoT(), etcdClientFactory, targetNode.Name)
			if err != nil {
				if !strings.Contains(err.Error(), "failed to check healthiness condition") {
					return nil
				}
			}
			return err
		}, memberIsDownTimeout, pollInterval).ShouldNot(o.HaveOccurred(), fmt.Sprintf("Node %s should have left the cluster", targetNode.Name))
